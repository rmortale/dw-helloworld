...
get_instance_tags () {
    ...
    echo $(/usr/bin/aws ec2 describe-tags --region ${region%?} --filters "Name=resource-id,Values=$instance_id")
}

get_ami_tags () {
    ami_id=$(/usr/bin/curl --silent http://169.254.169.254/latest/meta-data/ami-id)
    echo $(/usr/bin/aws ec2 describe-tags --region ${region%?} --filters "Name=resource-id,Values=$ami_id")
}
